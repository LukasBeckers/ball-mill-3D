...
class Detector:
    ...
    def __init__(
        ...
        max_dist_sticker=10,
        max_dist_ball=40,
        warmup_steps_ball=10,
    ):
        self.yolo_model = YOLO(f"../weights/{model_name}/best.pt")
        self.ballDetector = ballDetector(
            max_dist=max_dist_ball, warmup_steps=warmup_steps_ball
        )
        self.stickerDetector = stickerDetector(
            n_stickers=n_stickers, max_dist=max_dist_sticker
        )
    ...
    def __call__(self, image, mirror, return_yolo_output=False):
        ...
        return (
            (ball_results, sticker_results, detection_results)
            if return_yolo_output
            else (ball_results, sticker_results)
        )
    ...
    def __call__(self, detection_results, mirror=False, verbose=False):
        ...
        for i, (cl, box) in enumerate(zip(classes, boxes)):
            ...
            if cl == 1:  # 0 = Ball, 1 = Red Sticker
                coord = (
                    np.array([box[0] + box[2], box[1] + box[3]]) / 2
                )  # Box = xyxy creating average coord
        ...
        if self.first_frame:
            ...
            for i, roi in self.ROIS.items():
                ...
                if min_dist > self.max_dist:
                    if verbose:
                        print(
                            f"Over max dist in sticker detection! ROI: {i}, {roi} min-dist: {min_dist}, threshold: {self.max_dist}"
                        )
                    return None
                else:
                    results[i] = candidate
        ...
        if self.current_warmup_step == self.warmup_steps:
            ...
            diffs = [
                ...
                for [[x1, y1], [x2, y2]] in zip(
                    self.warmup_regions[:-1], self.warmup_regions[1:]
                )
            ]
    ...
    def __call__(self, detection_results, verbose=False):
        ...
        if self.current_warmup_step <= self.warmup_steps:
            ...
            detection_results = detection_results[
                0
            ]  # We only use single frame detection.
            boxes = detection_results.boxes.xyxy.tolist()
            classes = detection_results.boxes.cls
            ...
            for cl, box in zip(classes, boxes):
                ...
                if coord is not None:
                    ...
                    dist = np.sqrt(
                        (coord[0] - self.ROI[0]) ** 2 + (coord[1] - self.ROI[1]) ** 2
                    )
                    if dist > self.max_dist:
                        self.max_dist += 10
                        if verbose:
